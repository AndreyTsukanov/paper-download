...
class UnpaywallResolver:
    ...
    def resolve_oa_pdf_urls(self, doi: str) -> List[str]:
        """All OA PDF URLs for a DOI, repository hosts first (grab modification).

        Unpaywall often lists several OA locations; a publisher 'bronze' link may
        403 while a repository copy downloads fine. Returns every candidate URL
        (url_for_pdf preferred over url), ordered host_type=='repository' first,
        de-duplicated. resolve_best_pdf_url is left unchanged for other callers.
        """
        doi = (doi or "").strip()
        ...
        try:
            data = self._fetch_doi_record(doi)
        except Exception as exc:
            logger.warning("Unexpected Unpaywall resolver error for DOI %s: %s", doi, exc)
            return []
        if not data:
        ...
        def is_repo(loc):
            return (loc.get("host_type") or "").lower() == "repository"
