...
def _is_valid_pdf(file_path: str, min_size: int = 256) -> bool:
    """Check that a file exists, exceeds min_size bytes, and starts with %PDF magic."""
    try:
        if not os.path.exists(file_path):
            return False
        if os.path.getsize(file_path) < min_size:
            logger.warning("File too small to be a valid PDF: %s (%d bytes)", file_path, os.path.getsize(file_path))
            return False
        with open(file_path, "rb") as f:
            header = f.read(5)
        if not header.startswith(b"%PDF"):
            logger.warning("File does not start with %%PDF magic: %s (starts with %s)", file_path, header[:20])
            return False
        return True
    except Exception as exc:
        logger.warning("PDF validation failed for %s: %s", file_path, exc)
        return False


# Paywalled publisher DOI prefixes — OA chain will almost never succeed.
# Based on major commercial publishers that typically require subscriptions.
_PAYWALLED_DOI_PREFIXES: Dict[str, str] = {
    "10.1007": "Springer",
    "10.1002": "Wiley",
    "10.1016": "Elsevier",
    ...
    "10.1103": "APS (American Physical Society)",
    "10.1109": "IEEE",
    "10.1111": "Wiley",
    ...
    "10.1161": "AHA (American Heart Association)",
    "10.1177": "SAGE",
    "10.1186": "BioMed Central (Springer)",
    "10.1200": "ASCO (American Society of Clinical Oncology)",
    "10.1212": "AAN (American Academy of Neurology)",
    "10.1242": "Company of Biologists",
    "10.1371": "PLOS",
    "10.1523": "Society for Neuroscience",
    "10.1530": "Bioscientifica",
    "10.2105": "APHA (American Public Health Association)",
    "10.21037": "AME Publishing",
    "10.3389": "Frontiers",
    "10.3892": "Spandidos Publications",
}


def _is_likely_paywalled(doi: str) -> Optional[str]:
    """Return publisher name if DOI belongs to a known paywalled publisher, else None."""
    if not doi:
        return None
    normalized = doi.strip().lower()
    for prefix, publisher in _PAYWALLED_DOI_PREFIXES.items():
...
def _extract_pdf_link(html_bytes: bytes, base_url: str) -> Optional[str]:
    """Find a PDF link on an OA landing page (grab modification, not upstream).

    Tries the Highwire `citation_pdf_url` meta tag first, then a DSpace-style
    bitstream/download link or a plain `.pdf` href. Relative URLs are resolved
    ...
    try:
        ...
        for anchor in soup.find_all("a", href=True):
            ...
            low = href.lower()
            if low.endswith(".pdf") or "/bitstream" in low or low.rstrip("/").endswith("/download"):
...
async def _download_from_url_once(pdf_url: str, output_path: str, proxy: Optional[str] = None) -> Optional[str]:
    """One attempt to fetch a PDF (following an OA landing page once), optionally
    through a proxy. Returns the saved path or None. Caller handles fallback."""
    try:
        async with httpx.AsyncClient(follow_redirects=True, timeout=30, proxy=proxy) as client:
            response = await client.get(pdf_url)
            if response.status_code >= 400 or not response.content:
                return None

            if not _is_pdf_response(response):
                # OA landing page? Look for the actual PDF link and fetch it once.
                link = _extract_pdf_link(response.content, str(response.url))
                if not link or link == pdf_url:
                    content_type = (response.headers.get("content-type") or "").lower()
                    logger.warning("Resolved URL is not a PDF: %s (content-type=%s, starts=%s)", pdf_url, content_type, response.content[:20])
                    return None
                response = await client.get(link)
                if response.status_code >= 400 or not _is_pdf_response(response):
                    logger.warning("Landing-page PDF link was not a PDF: %s", link)
                    return None

        if len(response.content) < 256:
            logger.warning("Downloaded content too small (%d bytes), likely not a real PDF: %s", len(response.content), pdf_url)
            return None
        ...
        logger.warning("URL download failed for %s (proxy=%s): %s", pdf_url, bool(proxy), exc)
        return None


async def _download_from_url(pdf_url: str, save_path: str, filename_hint: str = "paper") -> Optional[str]:
    """Fetch a PDF, trying a direct connection first and falling back to the proxy
    (GRAB_DOWNLOAD_PROXY) only when direct fails. Some hosts (e.g. institutional
    repositories) are reachable only via the proxy, while most work direct — so we
    keep proxy use minimal and never make it the default path."""
    if not pdf_url:
        return None

    os.makedirs(save_path, exist_ok=True)
    output_path = os.path.join(save_path, f"{_safe_filename(filename_hint)}.pdf")

    result = await _download_from_url_once(pdf_url, output_path)
    if result:
        return result

    download_proxy = os.environ.get("GRAB_DOWNLOAD_PROXY")
    if download_proxy:
        logger.info("Direct download failed; retrying via proxy: %s", pdf_url)
        return await _download_from_url_once(pdf_url, output_path, proxy=download_proxy)
    return None
...
_TITLE_MATCH_RATIO = 0.75  # fallback when a candidate has no DOI to compare
...
def _title_similar(a: str, b: str) -> bool:
    a_norm = " ".join(re.findall(r"[a-z0-9Ѐ-ӿ]+", html.unescape(a or "").lower()))
    b_norm = " ".join(re.findall(r"[a-z0-9Ѐ-ӿ]+", html.unescape(b or "").lower()))
    if not a_norm or not b_norm:
        return False
    return SequenceMatcher(None, a_norm, b_norm).ratio() >= _TITLE_MATCH_RATIO


def _candidate_matches(paper, doi: str, title: str) -> bool:
    """True only if a repository candidate is the requested paper.

    Prefer an exact DOI match; if the candidate carries no DOI, fall back to a
    high title-similarity threshold. Anything else is rejected so we never save
    a topically-similar but different paper. (grab modification, not upstream.)
    """
    requested_doi = _normalize_doi(doi)
    candidate_doi = _normalize_doi(getattr(paper, "doi", "") or "")
    if requested_doi:
        if candidate_doi:
            return candidate_doi == requested_doi
        return _title_similar(title, getattr(paper, "title", "") or "")
    return _title_similar(title, getattr(paper, "title", "") or "")
    ...
    for repo_name, searcher in repository_searchers:
        for query in query_candidates:
            ...
            for paper in papers:
                ...
                # DOI gate (grab): only download a candidate that is actually the
                # requested paper; otherwise a relevance search hands us a
                # topically-similar OA paper and we silently save the wrong PDF.
                if not _candidate_matches(paper, doi, title):
                    continue
...
async def download_with_fallback(
    ...
    if source_name in primary_downloaders:
        try:
            ...
            # This connector has no direct PDF download by design (e.g. CrossRef is a
            # metadata registry, not a full-text host). That is expected, not a failure:
            # fall through to the OA chain quietly instead of logging a scary "download
            # failed" warning and polluting the honest error list with a constant message.
            logger.debug("No direct PDF from %s/%s (expected); using OA fallback: %s",
                         source_name, paper_id, exc)
        except Exception as exc:
    ...
    if normalized_doi:
        # Unpaywall may list several OA locations; a publisher 'bronze' link often
        # 403s while a repository copy downloads fine. Try them all, repository first.
        unpaywall_urls = await asyncio.to_thread(unpaywall_resolver.resolve_oa_pdf_urls, normalized_doi)
        if unpaywall_urls:
